...
public class continueSpeedline extends StarMacro {
	
  private static String speedlineDirectory;
  private static double mdot = 0.0;
  private static double dM = 0.1;  //Percent of starting mass flow rate. If going up, the value should be positive. If going down the value should be negative
 
  public void execute() {
	//Finishes the simulation that is given in the continueSpeedline.slurm file
	continue_();
	
	//Each instance of these runs a simulation at an additional point
	runMassOutlet(4000);
	runMassOutlet(4000);
	runMassOutlet(4000);
	runMassOutlet(4000);
  ...
  private void continue_() {
	...
	speedlineDirectory = simPath.getParent().toString();
  ...
  private void getMdot() {
    ...
    MassFlowReport massFlowReport_0 = 
      ((MassFlowReport) sim.getReportManager().getReport("Mass Flow P2"));

    mdot = massFlowReport_0.getReportMonitorValue();
